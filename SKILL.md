...
description: |
  Codex 专用 AI 视觉物料生产：内容结构化→品牌资产协议→风格选型→内置 image_gen 逐张生图→检查修正→PPTX / Keynote / HTML 图片 deck / 公众号封面 / 公众号正文配图 / B站·YouTube 视频封面 交付。优先使用 Codex 内置 `image_gen`，不依赖 nano banana、Gemini API 或本地 OPENAI_API_KEY——这是本 skill 存在的全部理由（省 API 成本）。
  当用户提到"做PPT"、"制作幻灯片"、"演示文稿"、"Keynote"、"slides"、"课件"、"汇报材料"、"路演"、"pptx"、"deck"、"图片PPT"时使用。
  当用户提到"公众号封面"、"公众号头图"、"公众号配图"、"正文插图"、"头条封面"、"B站封面"、"视频封面"、"YouTube封面"、"视频缩略图"、"做张封面"时同样使用——本 skill 把这些单图任务和 slides 共享同一套品牌资产协议、风格选型和 image_gen 路径。
  当用户提到"花叔风格"、"个人品牌"、"像素风"、"花叔的公众号头图"、"做成像素风"时，自动把本 skill `assets/personal-brand/` 里的像素头像/像素公众号头图示例/像素品牌资产作为 image_gen 参考图融入。
---

# Huashu Slide Codex

> 设计哲学：先像 `huashu-design` 一样完成设计判断（包含品牌资产协议），再像 Codex 一样把判断执行成一整套图片 PPT 或单张商单配图。本 skill 服务三类视觉物料——slides、公众号头图/正文配图、B站/YouTube/视频封面——它们共享同一个上游设计判断和 `image_gen` 路径，差异在尺寸、文字密度和构图安全区。
...
- `huashu-wechat-image` / `huashu-xhs-image`：常规环境下的公众号 / 小红书配图业务流程（Gemini API / Playwright / ImgBB 上传）。**当 agent 在 Codex 环境时，公众号头图/正文配图/视频封面这部分能力直接走本 skill 的 Path 4**——同样的标准（尺寸、文字规则、安全区、双钩子互补），但用 Codex 内置 `image_gen` 取代外部 API 调用。
